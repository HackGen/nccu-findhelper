(function () {
    var me, ref$, width, height, uiTest, charge, color, force, svg, gc1, gc2, mpl, reloadWorkaround, circleBox, lineBox, customDrag, currentDomain, clear, tmp2d3, playstate, depthvalue, depthmap, lockstate, nodes, gravitystate, generate, randomizer, updateSelect, initDb, init, headPayload, headName, headIconSelect, fu, join$ = [].join;
        ...
        nodes.each(function (d, i) {
            if (d.name == me.name) {
                ...
                if (d.fixed) {
                    d.px = 650;
                    d.py = 250;
                    d3.select(this).selectAll('circle').attr('stroke', '#f00').attr('r', '50');
                } else {
                    d3.select(this).selectAll('circle').attr('stroke', '#999').attr('r', '30');
                }
                return false;
            }
        });
    ...
    generate = function (error, graph) {
        var i$, ref$, len$, x;
        gc2 = svg.append('circle').attr('cx', width / 2).attr('cy', height / 2).attr('r', height).attr('fill', 'none').style('opacity', '0');
        gc1 = svg.append('circle').attr('cx', width / 2).attr('cy', height / 2).attr('r', 3 * height / 4).attr('fill', 'none').style('opacity', '0');
    ...
    initDb = function (domain) {
        ...
        $.getJSON("temp/testdata5.json", function (data) {

            reloadWorkaround = reloadWorkaround + 1;

            var reldata = window.relationData;

            me = data.me;

            if (!window.nameHash[data.me.name]) {
                reldata.nodes.push(data.me);
            }

            window.nameHash[data.me.name] = data.me;

            $.each(data.node, function (key, val) {

                if (!window.nameHash[val.name]) {
                    reldata.nodes.push(val);
                }

                window.nameHash[val.name] = val;


            });

            $.each(data.links, function (key, val) {
                if (!window.linkHash[val.name]) {
                    reldata.links.push(val);
                }

                window.linkHash[val.src + " " + val.name + " " + val.des] = val;
            ...
            window.relationData = reldata;
            updateSelect(reldata);
            window.updateRelations(reldata);
            $(".text-relation").siblings("rect").attr("fill", "none");
